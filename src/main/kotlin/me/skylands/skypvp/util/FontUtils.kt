...
enum class FontUtils(val character: Char, val length: Int) {
    ...
    companion object {
        fun getDefaultFontInfo(c: Char): FontUtils {
            for (dFI in FontUtils.values()) {
        ...
        private const val MOTD_PX = 130
        fun sendCenteredMessage(sender: CommandSender, message: String) {
            sender.sendMessage(centerMessage(message, CENTER_PX))
        }

        fun centerMotd(message: String): String {
            return centerMessage(message, MOTD_PX)
        }

        fun centerMessage(message: String, length: Int): String {
            var message = message
            ...
            for (c in message.toCharArray()) {
                if (c == '§') {
                    ...
                    val dFI = getDefaultFontInfo(c)
                    messagePxSize += if (isBold) dFI.boldLength else dFI.length
                    messagePxSize++
                }
            }
            val halvedMessageSize = messagePxSize / 2
            val toCompensate = length - halvedMessageSize
            val spaceLength = SPACE.length + 1
            var compensated = 0
